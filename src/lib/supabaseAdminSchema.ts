...
-- Cada escola possui seus próprios administradores (rufus_admin.escola_id).
...
-- Superusuário: pode criar escolas pelo painel (is_super = true; sem escola fixa)
ALTER TABLE rufus_admin ADD COLUMN IF NOT EXISTS is_super BOOLEAN NOT NULL DEFAULT false;
-- Unicidade do e-mail entre superusuários (escola_id é NULL para eles, então a UNIQUE (email, escola_id)
-- não colide). Sem isso, rodar o script de novo duplicaria o superusuário.
DO $do$
BEGIN
  IF NOT EXISTS (
...
-- Superusuário RUFUS (cria escolas no painel). Não pertence a uma escola específica.
...
-- Cria uma nova escola + seu primeiro administrador. SOMENTE o superusuário executa.
...
DECLARE
  ...
  v_escola_id UUID;
BEGIN
  -- 1. Valida o superusuário (email + senha)
  SELECT * INTO sup FROM rufus_admin
  WHERE lower(email) = lower(p_email_super) AND is_super = true;
  IF NOT FOUND THEN
    RETURN jsonb_build_object('ok', false, 'msg', 'Apenas o superusuário pode criar escolas.');
  END IF;
  IF sup.senha_hash <> crypt(p_senha_super, sup.senha_hash) THEN
    RETURN jsonb_build_object('ok', false, 'msg', 'Senha do superusuário incorreta.');
  END IF;

  -- 2. Valida campos
  IF length(coalesce(p_nome_escola, '')) < 3 THEN
    RETURN jsonb_build_object('ok', false, 'msg', 'Informe o nome da escola (mínimo 3 caracteres).');
  END IF;
  ...
  -- 4. Cria o primeiro administrador da escola (senha inicial 000000 — troca no 1º acesso)
