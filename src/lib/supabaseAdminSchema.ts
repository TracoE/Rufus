...
-- 6. CÓDIGOS DE INSTALAÇÃO DOS TERMINAIS (por escola, gerados pelo SUPER).
-- O terminal NÃO lista as escolas: no 1º uso ele pede apenas o código, que é
-- resolvido pelo RPC abaixo num único resultado (não há enumeração).
-- Só o super (ou admin da escola) lê/gera/exclui os códigos.
...
-- Gera um código único para uma escola (apenas superusuário).
...
-- Resolve um código para a escola (usado pelo instalador do terminal, sem login).
-- Responsável por NÃO listar nada: retorna apenas a escola do código informado.
